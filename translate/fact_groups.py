...
def expand_group_by_type(group, task, reachable_facts):
    result = []
    result_set = set()
    for fact in group:
        try:
            ...
            for obj in task.objects:
                ...
                if atom in reachable_facts:
                    result.append(atom)
                    newargs[pos] = obj.type
                    result_set.add(pddl.Atom(fact.predicate, newargs))
    if len(result) == 1: # only a ground facts 
        return set()
    return result_set
...
def instantiate_groups_by_types(groups, task, reachable_facts):
    return [expand_group_by_type(group, task, reachable_facts) for group in groups]
...
def compute_abs_groups(task, atoms, reachable_action_params,partial_encoding = True):
    groups = invariant_finder.get_groups(task, reachable_action_params)

    with timers.timing("Instantiating groups"):
        groups = instantiate_groups_by_types(groups, task, atoms)
